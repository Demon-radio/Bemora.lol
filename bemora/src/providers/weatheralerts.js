...
export async function getUSAlerts({ state } = {}) {
  ...
  const { data } = await axios.get(url, { headers: { 'User-Agent': 'bemora-npm-library' } });
...
export async function getPointForecast({ lat, lon }) {
  const { data: point } = await axios.get(`https://api.weather.gov/points/${lat},${lon}`, { headers: { 'User-Agent': 'bemora-npm-library' } });
  const forecastUrl = point.properties?.forecast;
  if (!forecastUrl) throw new Error('No forecast available for this location');
  const { data: forecast } = await axios.get(forecastUrl, { headers: { 'User-Agent': 'bemora-npm-library' } });
