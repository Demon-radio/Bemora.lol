...
export async function getNHLTeams() {
  ...
  try {
    const { data } = await http.get('https://api-web.nhle.com/v1/standings/now');
    ...
    throw wrapProviderError(err, 'hockey');
  }
}

export async function getNHLPlayer({ id }) {
  try {
    const { data } = await http.get(`https://api-web.nhle.com/v1/player/${id}/landing`);
    return {
      player: {
        id: data.playerId,
