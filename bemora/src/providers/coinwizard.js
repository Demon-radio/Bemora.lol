...
export async function coinInfo({ id }) {
  ...
  try {
    const { data } = await http.get(`${BASE}/coins/${encodeURIComponent(id)}`, {
    ...
    throw wrapProviderError(err, 'coinwizard');
  }
}
...
export async function marketChart({ id, days = 7, vsCurrency = 'usd' }) {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/coins/${encodeURIComponent(id)}/market_chart`, {
...
export async function ohlc({ id, days = 7, vsCurrency = 'usd' }) {
  try {
    const { data } = await http.get(`${BASE}/coins/${encodeURIComponent(id)}/ohlc`, { params: { vs_currency: vsCurrency, days } });
...
export async function globalMarket() {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/global`);
    const d = data.data;
    const result = {
...
export async function exchanges({ limit = 10 } = {}) {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/exchanges`, { params: { per_page: limit, page: 1 } });
...
export async function categories({ limit = 15 } = {}) {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/coins/categories`);
...
export async function searchCoins({ query }) {
  try {
    const { data } = await http.get(`${BASE}/search`, { params: { query } });
    return {
...
export async function convertCoin({ id, amount = 1, vsCurrency = 'usd' }) {
  try {
    const { data } = await http.get(`${BASE}/simple/price`, { params: { ids: id, vs_currencies: vsCurrency } });
...
export async function supportedCoinsList() {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/coins/list`);
