...
export async function searchTeam({ name }) {
  ...
  try {
    const { data } = await http.get(`${BASE}/searchteams.php`, { params: { t: name } });
    ...
    throw wrapProviderError(err, 'sportsdb');
  }
}

export async function searchPlayer({ name }) {
  try {
    const { data } = await http.get(`${BASE}/searchplayers.php`, { params: { p: name } });
...
export async function getLeagueEvents({ leagueId }) {
  try {
    const { data } = await http.get(`${BASE}/eventsnextleague.php`, { params: { id: leagueId } });
...
export async function listLeagues({ sport } = {}) {
  ...
  if (cached) return { ...cached, _cached: true };
  try {
    const { data } = await http.get(`${BASE}/all_leagues.php`);
