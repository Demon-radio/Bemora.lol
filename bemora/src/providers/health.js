...
export async function searchDrug({ name, limit = 5 }) {
  ...
  try {
    const { data } = await http.get('https://api.fda.gov/drug/label.json', {
      params: {
    ...
    throw wrapProviderError(err, 'health');
  }
}
...
export async function getExercises({ muscle, equipment, limit = 10 } = {}) {
  ...
  try {
    const { data } = await http.get('https://wger.de/api/v2/exercise/', {
      params: {
...
export async function getNutrition({ query }) {
  ...
  try {
    const { data } = await http.get('https://world.openfoodfacts.org/cgi/search.pl', {
